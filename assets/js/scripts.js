...
window.addEventListener('scroll', () => {
    ...
    // Scrolling down
    if (currentScrollPosition < previousScrollPosition && previousScrollPosition > 40) {
...
// Load comments
const commentsButton = document.getElementById("js-comments");

if (commentsButton) {
    commentsButton.addEventListener("click", () => {
        commentsButton.classList.toggle("is-hidden");
...
(function() {
   ...
   if (
       ...
       if (showAfterTime) {
           setTimeout(() => {
               if (
                   !newsletter.classList.contains('is-visible') &&
                   localStorage.getItem('newsletter-subscribe') !== 'true'
               ) {
                   newsletter.classList.add('is-visible');
               }
